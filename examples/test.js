const { DexchangeClient } = require('@dexchange/sdk');
...
async function testerSDK() {
  try {
    ...
    const transactionWave = await client.transaction.init({
      externalTransactionId: `ORDER-${Date.now()}`,
      serviceCode: 'WAVE_SN',
      amount: 2000,
      number: '771234567',
    ...
    const lienPaiement = await client.transaction.createMerchantPaymentLink({
      externalTransactionId: `ORDER-${Date.now()}`,
    ...
    if (transactionOM.transaction.transactionId) {
      ...
      const statut = await client.transaction.getTransaction(
    ...
    const solde = await client.services.getBalance();
    console.log('Solde:', solde);

    // 7. Exemple avec Wizall (si applicable)
    console.log('\n7. Test de confirmation Wizall:');
    const confirmationWizall = await client.transaction.confirmWizall({
...
function gererWebhook(signature, payload, secret) {
